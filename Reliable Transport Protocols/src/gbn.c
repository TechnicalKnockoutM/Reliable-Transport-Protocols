...
int base,num_next,num_expected,acknum_expected;
int start_window,end_window,num_packet;
int start_buffer,end_buffer,num_message;
struct pkt *window_buffer;//Buffer for storing packets sent
struct msg *buffer;	/* Buffer for storing messages which falls 						outside the window*/
...
void A_output(message)
	...
	if (num_next < base + WINSIZE) {/*Checking condition for 								packet that falls inside 								window*/
		acknum_expected = num_next;
		sender_packet.seqnum = num_next;
		sender_packet.acknum = 0;
		/*Copying message into packet's payload*/
		strncpy(sender_packet.payload, message.data, 20);
	...
	/*Storing packets in a buffer that are sent*/
		...
		window_buffer[end_window] = sender_packet;
		for (int i = 0; i < 20; i++) {
			window_buffer[end_window].payload[i] = message.data[i];
		}
	/*Condition for timer to start*/
		if (num_next == base) {
		/*Timer starts here*/
...
void A_input(packet)
	...
	if ((packet.checksum == Checksum(packet)) && Corrupted(packet) == 0) {
		printf("A_input ACK %d received ", packet.acknum);
		acknum_expected++;
		...
		/*Stop timer as we got ack for base*/
		if (base == num_next) {
			stoptimer(0);
		} else {
			stoptimer(0);
	...
	/* send  packets which were stored in message buffer */
		while ((num_message != 0) && (num_next < base + WINSIZE)) {
			...
			for (int i = 0; i < 20; i++)
				sender_packet.payload[i] = buffer[start_buffer].data[i];
			/* checksum */
			sender_packet.checksum = Checksum(sender_packet);
				/*Condition for timer to start*/
			if (base == num_next) {
...
void A_timerinterrupt() {
	...
	for (int i = 1; i <= num_packet; i++) {
		tolayer3(0, window_buffer[(start_window + i) % WINSIZE]);
		A_transport++;
	}
...
void A_init() {
/*Initializing varaibles to zero*/
	num_packet = 0;
	num_message = 0;
	acknum_expected = 0;
	base = 0;
...
/*Allocating memory to buffers*/
	window_buffer = (struct pkt *) malloc(sizeof(struct pkt) * WINSIZE);
...
/*Check for unexpected sequence number and Corrupted Packet*/
	if (packet.seqnum != num_expected && Corrupted(packet) == 0) {
	...
	strncpy(reciever_packet.payload, packet.payload, 20);
	reciever_packet.checksum = Checksum(reciever_packet);
	/*Sending ack to sender*/
	tolayer3(1, reciever_packet);
	num_expected++;/*Incrementing sequence number expected 					value*/
...
void B_init() {
	num_expected = 0;/*Initialize initial sequence number 						expected to 0*/
	
	/*This packet is if initial packet is lost and to send 	default acknowledgement*/
	reciever_packet.acknum = 0;
	reciever_packet.seqnum = 0;
	for (int i = 0; i < 20; i++) {
		reciever_packet.payload[i] = '0';
	}
	reciever_packet.checksum = reciever_packet.seqnum + reciever_packet.acknum;
	
	for (int i = 0; i < 20; i++) {
		reciever_packet.checksum += reciever_packet.payload[i];
	}
}
